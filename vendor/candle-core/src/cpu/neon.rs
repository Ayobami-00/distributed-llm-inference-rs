...
mod fp16 {
    ...
    // dlir v0.1 is F32-only. Keep Candle's portable widening implementation so
    // candle-core 0.11.0 compiles on stable Rust; its optimized branch below
    // names stdarch's still-unstable `float16x8_t` intrinsic.
    #[cfg(not(any()))]
    mod inner {
        use super::*;
