use crate::{
    backend::BackendStorage, CpuStorage, DType, Device, IsMultipleOfCompat, Result, Shape, Storage,
    Tensor, D,
};
use k_quants::*;
...
impl<T: k_quants::GgmlType + Send + Sync> QuantizedType for Vec<T> {
    ...
    fn embedding(&self, ids: &[u32], rows: usize, hidden: usize) -> Result<CpuStorage> {
        if !hidden.is_multiple_of_compat(T::BLCK_SIZE) {
            crate::bail!(
...
fn check_shape(shape: &Shape, block_size: usize) -> Result<()> {
    ...
    if !dims[dims.len() - 1].is_multiple_of_compat(block_size) {
        crate::bail!(
...
impl QTensor {
    ...
    pub fn quantize(src: &Tensor, dtype: GgmlDType) -> Result<Self> {
        ...
        if !elem_count.is_multiple_of_compat(block_size) {
            crate::bail!(
    ...
    pub fn embedding(&self, ids: &Tensor) -> Result<Tensor> {
        ...
        if !hidden.is_multiple_of_compat(self.dtype().block_size()) {
            crate::bail!(
                "quantized embedding hidden size {hidden} is not divisible by block size {}",
    ...
    fn cpu_fwd(
        ...
        match storage.dtype() {
            DType::F32 => {
                ...
                if self_storage.dtype() == GgmlDType::Q4K && n.is_multiple_of_compat(8) {
