...
// Compatibility shim for dlir's Rust 1.85 MSRV. Candle 0.11.0 uses the integer
// `is_multiple_of` method stabilized after that baseline.
pub(crate) trait IsMultipleOfCompat {
    fn is_multiple_of_compat(self, other: Self) -> bool;
}

macro_rules! impl_is_multiple_of_compat {
    ($($type:ty),+ $(,)?) => {
        $(
            impl IsMultipleOfCompat for $type {
                fn is_multiple_of_compat(self, other: Self) -> bool {
...
impl_is_multiple_of_compat!(u8, u16, u32, u64, u128, usize, i8, i16, i32, i64, i128, isize);
